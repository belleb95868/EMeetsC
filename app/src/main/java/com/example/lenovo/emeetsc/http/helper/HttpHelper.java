...
import com.example.lenovo.emeetsc.http.handler.OkMainHandler;
...
class HttpHelper extends BaseHelper{
    ...
    private Request buildRequest(HttpInfo info, @RequestMethod int method){
        ...
        if(method == RequestMethod.POST){
            ...
            else if(info.getJsonStr()!=null||!info.getJsonStr().equals("")){
                RequestBody requestBody=RequestBody.create(MEDIA_TYPE_MARKDOWN,info.getJsonStr());
                requestBuilder.url(url).post(requestBody);
            }
            else requestBuilder.url(url).post(builder.build());
